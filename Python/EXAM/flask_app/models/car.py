...
class Car:
    ...
    @classmethod
    def get_all(cls):
        query = "SELECT * FROM sellers JOIN cars ON sellers.id = cars.sellers_id"
    ...
    def pickall(cls, data):
        query = "SELECT * FROM cars JOIN sellers ON sellers.id = cars.sellers_id WHERE sellers_id = %(id)s"
        return connectToMySQL(DB).query_db(query, data)
